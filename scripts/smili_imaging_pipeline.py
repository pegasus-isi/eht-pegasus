...
parser.add_argument(
    "--day",
    metavar="observing day of April 2017",
    type=int,
    required=True,
...
parser.add_argument(
    "--sys",
    metavar="fractional systematic error",
    type=float,
    default=0.0,
...
parser.add_argument(
    "--lambl1", metavar="lambda l1", type=float, default=1.0, help="lambda l1"
)
parser.add_argument(
    "--lambtv", metavar="lambda tv", type=float, default=1e3, help="lambda tv"
)
parser.add_argument(
    "--lambtsv", metavar="lambda tsv", type=float, default=1e4, help="lambda tsv"
)
parser.add_argument(
    "--nproc", metavar="Number of parallel processors. Default=1.", type=int, default=1
)
...
if day not in [5, 6, 10, 11]:
    raise ValueError(
        "The observational day (--day integer) must be (April) 5, 6, 10 or 11."
    )
...
def gen_prior_gauss(x0=0.0, y0=0.0, fwhm=40.0, dx=dx_uas, nx=nx):
    """
...
obs = obs.avg_coherent(0.0, scan_avg=True)  # average each scan coherantly
...
if "LM" in obs.data["t1"] or "LM" in obs.data["t2"]:
    ...
    gausspriormodel = eh.image.make_square(
        obs, int(nx), nx * dx_uas * eh.RADPERUAS
    )  # Create empty image
    gausspriormodel = gausspriormodel.add_gauss(
        0.6, (60 * eh.RADPERUAS, 60 * eh.RADPERUAS, 0, 0, 0)
    )  # Add gaussian
...
if not args.keepsinglepol:
    for stname in singlepol:
        vtable = vtable.query(
            "st1name != '" + stname + "' and st2name != '" + stname + "'"
...
for iself in range(Nself):
    ...
    initimage = initimage.add_gauss(
        totalflux=totalflux, majsize=prior_fwhm, overwrite=True
    )
    ...
    for iweig in range(Nweig):
        ...
        if iself < Nflcl:
            ...
            atable.loc[idx, "sigma"] = np.sqrt(
                atable.loc[idx, "sigma"] ** 2 + (atable.loc[idx, "amp"] * 0.3) ** 2
            )
            #  For other stations (adding an error specified with amp_err)
            idx = idx == False
            atable.loc[idx, "sigma"] = np.sqrt(
                atable.loc[idx, "sigma"] ** 2 + (atable.loc[idx, "amp"] * 0.05) ** 2
            )
