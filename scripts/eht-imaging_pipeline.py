...
parser.add_argument(
    ...
    "--infile2",
    default="",
    help="optional 2nd input file (different band) for imaging",
)
parser.add_argument("-o", "--outfile", default="out.fits", help="output FITS image")
parser.add_argument(
    "--savepdf",
    default=False,
    help="saves image pdf (True or False)?",
    action="store_true",
)
parser.add_argument(
    "--imgsum", default=False, help="generate image summary pdf", action="store_true"
)
...
gain_tol = [
    0.02,
    0.2,
]  # Asymmetric gain tolerance for self-cal; we expect larger values
...
# but do not form closure quantities between the two datasets
    ...
    obs1.add_scans()
    obs2.add_scans()
    obs1 = obs1.avg_coherent(0.0, scan_avg=True)
    obs2 = obs2.avg_coherent(0.0, scan_avg=True)
...
gaussprior = gaussprior.add_gauss(
    zbl * 1e-3, (prior_fwhm, prior_fwhm, 0, prior_fwhm, prior_fwhm)
)
...
obs_LMT = obs_sc_init.flag_uvdist(
    uv_max=2e9
)  # only consider the short baselines (LMT-SMT)
if reverse_taper_uas > 0:
...
gausspriorLMT = gausspriorLMT.add_gauss(
    0.6, (60.0 * eh.RADPERUAS, 60.0 * eh.RADPERUAS, 0, 0, 0)
)
...
obs_sc = eh.selfcal(
    obs_sc, imgr.out_last(), method="phase", ttype=ttype, solution_interval=0.0
)
...
for repeat_selfcal in range(2):
    ...
    imgr = eh.imager.Imager(
        ...
        prior_im=gaussprior,
        flux=zbl,
        data_term=data_term_final,
        maxit=maxit,
        norm_reg=True,
        systematic_noise=0.01,
        reg_term=reg_term,
        ttype=ttype,
        cp_uv_min=uv_zblcut,
        stop=stop,
