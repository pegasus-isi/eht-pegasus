...
parser = argparse.ArgumentParser(
    description="Script to convert EHT fits files to pdf images"
)
parser.add_argument("-i", "--infile", default="", help="input FITS file")
parser.add_argument("-o", "--obsfile", default="", help="input UVFITS file")
parser.add_argument(
    "-r", "--regrid", default=False, help="regrid image object", action="store_true"
)
parser.add_argument(
    "-c", "--center", default=False, help="center image object", action="store_true"
)
parser.add_argument(
    "-s", "--scale", default=False, help="display scale in output", action="store_true"
)
parser.add_argument(
    "-b",
    "--beam",
    default=False,
    help="display beam size in output",
    action="store_true",
)
parser.add_argument(
    "-a",
    "--all",
    default=False,
    help="perform all post-processing steps",
    action="store_true",
)
parser.add_argument(
    "-n",
    "--notitle",
    default=False,
    help="remove title and colorbar",
    action="store_true",
)
parser.add_argument("-O", "--outfile", default="difmap-imgsums", help="output PDF file")
...
if args.center or args.all:
    im_obj = im_obj.shift([8, 0])  # Not an exact value, but a close estimate
if args.regrid or args.all:
    im_obj = im_obj.regrid_image(
        128 * eh.RADPERUAS, 192
    )  # im_obj.regrid_image(160*eh.RADPERUAS, 1920) # 128->192, 64->128->192
params = [
    9.696e-11,
    9.696e-11,
    0,
]  # This is for DIFMAP (20 uas) -- do NOT blur, as it is already applied. This is used only for display()’s beamparams
...
if zbl > zbl_tot:
    print(
        "Warning: Specified total compact flux density "
...
outimgsum = args.outfile + "/" + args.infile[:-4] + "img.pdf"
